...
class CellposeModel(UnetModel):
    ...
    def _run_cp(self, x, compute_masks=True, normalize=True, invert=False,
        ...
        for i in iterator:
            ...
            print('number of styles:', style_num)
            #stylei = np.zeros((style_num,3,224,224))
            styles_list = []
            ...
            h_s,w_s = int(h_ori*0.8),int(w_ori*0.8)
            h_m,w_m = int(h_ori*1.5),int(w_ori*1.5)
            h_l,w_l = int(h_ori*2.0),int(w_ori*2.0)
            image_l = cv2.resize(img.copy(), dsize=(w_l, h_l), interpolation=cv2.INTER_CUBIC)
            image_m = cv2.resize(img.copy(), dsize=(w_m, h_m), interpolation=cv2.INTER_CUBIC)
            image_s = cv2.resize(img.copy(), dsize=(w_s, h_s), interpolation=cv2.INTER_CUBIC)
            imgs_l,imgs_s, imgs_m = [image_l],[image_s],[image_m]
                
            ori = imgs_o[0]
            r1 = cv2.rotate(ori, cv2.ROTATE_90_CLOCKWISE)
            ...
            lar = imgs_l[0]
            r1 = cv2.rotate(lar, cv2.ROTATE_90_CLOCKWISE)
            r2 = cv2.rotate(r1, cv2.ROTATE_90_CLOCKWISE)
            r3 = cv2.rotate(r2, cv2.ROTATE_90_CLOCKWISE)
            imgs_l.append(r1)
            imgs_l.append(r2)
            ...
            mid = imgs_m[0]
            r1 = cv2.rotate(mid, cv2.ROTATE_90_CLOCKWISE)
            r2 = cv2.rotate(r1, cv2.ROTATE_90_CLOCKWISE)
            r3 = cv2.rotate(r2, cv2.ROTATE_90_CLOCKWISE)
            imgs_m.append(r1)
            imgs_m.append(r2)
            ...
            sma = imgs_s[0]
            r1 = cv2.rotate(sma, cv2.ROTATE_90_CLOCKWISE)
            r2 = cv2.rotate(r1, cv2.ROTATE_90_CLOCKWISE)
            r3 = cv2.rotate(r2, cv2.ROTATE_90_CLOCKWISE)
            imgs_s.append(r1)
            imgs_s.append(r2)
            ...
            if normalize or invert:
                for j in range(len(imgs_o)):
                    imgs_o[j] = transforms.normalize_img(imgs_o[j], invert=invert)
                for j in range(len(imgs_l)):
                    imgs_l[j] = transforms.normalize_img(imgs_l[j], invert=invert)
                for j in range(len(imgs_m)):
                    imgs_m[j] = transforms.normalize_img(imgs_m[j], invert=invert)
                for j in range(len(imgs_s)):
                    imgs_s[j] = transforms.normalize_img(imgs_s[j], invert=invert)
            if rescale != 1.0:
                for j in range(len(imgs_o)):
                    imgs_o[j] = transforms.resize_image(imgs_o[j], rsz=rescale)
                for j in range(len(imgs_l)):
                    imgs_l[j] = transforms.resize_image(imgs_l[j], rsz=rescale)
                for j in range(len(imgs_m)):
                    imgs_m[j] = transforms.resize_image(imgs_m[j], rsz=rescale)
                for j in range(len(imgs_s)):
                    imgs_s[j] = transforms.resize_image(imgs_s[j], rsz=rescale)
            yf_o = []
            yf_l = []
            yf_s = []
            yf_m = []
            ...
            for j in range(len(imgs_o)): 
                for s in styles_list:
                    yf_, style = self._run_nets(imgs_o[j], s, net_avg=net_avg,augment=augment, tile=tile,tile_overlap=tile_overlap)
                    yf_o.append(yf_)
            for j in range(len(imgs_l)): 
                for s in styles_list:
                    yf_, style = self._run_nets(imgs_l[j], s,net_avg=net_avg,augment=augment, tile=tile,tile_overlap=tile_overlap)
                    yf_l.append(yf_)
            for j in range(len(imgs_m)):     
                for s in styles_list:
                    yf_, style = self._run_nets(imgs_m[j],s, net_avg=net_avg,augment=augment, tile=tile,tile_overlap=tile_overlap)
                    yf_m.append(yf_)
                
            for j in range(len(imgs_s)):   
                for s in styles_list:
                    yf_, style = self._run_nets(imgs_s[j], s, net_avg=net_avg,augment=augment, tile=tile,tile_overlap=tile_overlap)
                    yf_s.append(yf_)

            for j in range(len(styles_list)):
                yf_o[j+3] = cv2.rotate(yf_o[j+3], cv2.ROTATE_90_COUNTERCLOCKWISE)
                yf_o[j+6] = cv2.rotate(yf_o[j+6], cv2.ROTATE_180)
                yf_o[j+9] = cv2.rotate(yf_o[j+9], cv2.ROTATE_90_CLOCKWISE)
                
                yf_l[j+3] = cv2.rotate(yf_l[j+3], cv2.ROTATE_90_COUNTERCLOCKWISE)
                yf_l[j+6] = cv2.rotate(yf_l[j+6], cv2.ROTATE_180)
                yf_l[j+9] = cv2.rotate(yf_l[j+9], cv2.ROTATE_90_CLOCKWISE)
                
                yf_m[j+3] = cv2.rotate(yf_m[j+3], cv2.ROTATE_90_COUNTERCLOCKWISE)
                yf_m[j+6] = cv2.rotate(yf_m[j+6], cv2.ROTATE_180)
                yf_m[j+9] = cv2.rotate(yf_m[j+9], cv2.ROTATE_90_CLOCKWISE)
                
                yf_s[j+3] = cv2.rotate(yf_s[j+3], cv2.ROTATE_90_COUNTERCLOCKWISE)
                yf_s[j+6] = cv2.rotate(yf_s[j+6], cv2.ROTATE_180)
                yf_s[j+9] = cv2.rotate(yf_s[j+9], cv2.ROTATE_90_CLOCKWISE)
                
                
            if resample:
                for j in range(len(yf_o)):
                    yf_o[j] = transforms.resize_image(yf_o[j], shape[1], shape[2])
                for j in range(len(yf_l)):
                    yf_l[j] = transforms.resize_image(yf_l[j], shape[1], shape[2])
                for j in range(len(yf_m)):
                    yf_m[j] = transforms.resize_image(yf_m[j], shape[1], shape[2])
                for j in range(len(yf_s)):
                    yf_s[j] = transforms.resize_image(yf_s[j], shape[1], shape[2])
            #yf = yf_o[0].copy()
            yf_o_bi = []
            yf_l_bi = []
            yf_m_bi = []
            yf_s_bi = []
            for j in range(4):#iterate 4 orientations
                for k in range(len(styles_list)):#iterate all styles
                    yf_o_bi.append(yf_o[j*len(styles_list)+k] > 0) 
                    yf_l_bi.append(yf_l[j*len(styles_list)+k] > 0) 
                    yf_m_bi.append(yf_m[j*len(styles_list)+k] > 0) 
                    yf_s_bi.append(yf_s[j*len(styles_list)+k] > 0) 
            dices_total = []
            for yf_scales in ([yf_o_bi,yf_l_bi,yf_m_bi,yf_s_bi]):
                for k in range(len(styles_list)):
                    one = yf_scales[k][:,:,2].flatten()
                    two = yf_scales[k+3][:,:,2].flatten()
                    three = yf_scales[k+6][:,:,2].flatten()
                    four = yf_scales[k+9][:,:,2].flatten()
                    intersection1 = (one * two)
                    intersection2 = (one * three)
                    intersection3 = (one * four)
                    intersection4 = (two * three)
                    intersection5 = (two * four)
                    intersection6 = (three * four)
                    dice1 = (2. * intersection1.sum() + 1) / (one.sum() + two.sum() + 1)
                    dice2 = (2. * intersection2.sum() + 1) / (one.sum() + three.sum() + 1)
                    dice3 = (2. * intersection3.sum() + 1) / (one.sum() + four.sum() + 1)
                    dice4 = (2. * intersection4.sum() + 1) / (two.sum() + three.sum() + 1)
                    dice5 = (2. * intersection5.sum() + 1) / (two.sum() + four.sum() + 1)
                    dice6 = (2. * intersection6.sum() + 1) / (three.sum() + four.sum() + 1)
                    dices_total.append((dice1+dice2+dice3+dice4+dice5+dice6)/6)
                
            max_ind=0
            max_dice = 0
            for j in range(len(dices_total)):
                if dices_total[j]>max_dice:
                    if dices_total[j]==1: continue
                    max_dice=dices_total[j]
                    max_ind = j
            if max_ind>=0 and max_ind<=2:
                yf = yf_o[max_ind]
                print('pick original',max_ind)
            elif max_ind>=3 and max_ind<=5:
                yf = yf_l[max_ind-len(styles_list)]
                print('pick large',max_ind)
            elif max_ind>=6 and max_ind<=8:
                yf = yf_m[max_ind-len(styles_list)*2]
                print('pick middle',max_ind)
            else:
                print('pick small',max_ind)
                yf = yf_s[max_ind-len(styles_list)*3]
            cellprob[i] = yf[:,:,2]
