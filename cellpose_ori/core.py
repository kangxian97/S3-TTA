...
class UnetModel():
    def __init__(self, gpu=False, pretrained_model=False,pretrained_model_adain='',
        ...
        style_vgg.load_state_dict(torch.load('/content/drive/MyDrive/Hardvard_BC_research/AdalN/AdaIN/models/vgg_normalised.pth'))
        print('loaded pretrained vgg and pretrained ST decoder60000')
        style_decoder.load_state_dict(torch.load('/content/drive/MyDrive/Hardvard_BC_research/TransferBaseline1/adain_model/decoder_iter_116000.pth.tar'))
    ...
    def network(self, x, style_anchor, return_conv=False):
        ...
        x=x/255
        #print('x shape in network function:', x.shape)
        X = self._to_device(x)
        self.net.eval()
        if self.mkldnn:
        ...
        ########################
        save_img_dir = '/content/drive/MyDrive/Hardvard_BC_research/multiscale_co_training/transferred_images_smallcellanchor'
        save_pred_dir = '/content/drive/MyDrive/Hardvard_BC_research/multiscale_co_training/prediction_masks'
        with torch.no_grad():
            full_x = np.stack((x[:,0,:,:],x[:,0,:,:],x[:,0,:,:]),axis=1)
    ...
    def _run_nets(self, img, style_anchor,net_avg=False, augment=False, tile=True, tile_overlap=0.1, bsize=224, 
        ...
        if isinstance(self.pretrained_model, str) or not net_avg:  
            y, style = self._run_net(img,style_anchor, augment=augment, tile=tile, tile_overlap=tile_overlap,
            ...
            for j in range(len(self.pretrained_model)):
                ...
                y0, style = self._run_net(img,style_anchor, augment=augment, tile=tile, 
    ...
    def _run_net(self, imgs, style_anchor,augment=False, tile=True, tile_overlap=0.1, bsize=224,
                 return_conv=False):
...
#         slc = [slice(0, self.nclasses) for n in range(imgs.ndim)] # changed from imgs.shape[n]+1 for first slice size 
        ...
        if tile or augment or imgs.ndim==4:
            print('is run tiled')
            y, style = self._run_tiled(imgs, style_anchor,augment=augment, bsize=bsize, 
                                      tile_overlap=tile_overlap, 
                                      return_conv=return_conv)
        else:
            print('is run network')
            imgs = np.expand_dims(imgs, axis=0)
            y, style = self.network(imgs, style_anchor, return_conv=return_conv)
            y, style = y[0], style[0]
        ...
        if imgi.ndim==4:
            ...
            if ny*nx > batch_size:
                ...
                for k in ziterator:
                    ...
                    ya, stylea = self.network(IMGa,self.style_anchor_dir)
                    for i in range(min(Lz-k*nimgs, nimgs)):
            ...
            for k in range(niter):
                ...
                y0, style = self.network(IMG[irange],style_anchor, return_conv=return_conv)
    ...
    def _find_scale(self,imgi,styles, scales = [1,0.8,1.5,2]):
        #print('in find scale styles shape', styles.shape)
        #styles shape = (nstyles,3,h,w)
        ...
        for ind,(h,w) in enumerate(zip(h_,w_)):
            ...
            for style_ind in range(style_num):
                ...
                #print(ind*style_num+style_ind)
        #print("all dices:", all_dices)
        #print(all_dices.shape)
        #print(all_dices)
        ...
        #print(scale_selection)
        style_selction = selection%style_num
        #print(style_selction)
        ...
        #print('final_imgs shape:',final_imgs.shape)
        #print('final_styles shape:',final_styles.shape)
    ...
    def _train_step(self, x, lbl,styles):
        #print('in train step x shape:', x.shape) bs,224,224,1
        #x.shape = (batch,2,224,224)
        full_x = np.stack((x[:,:,:,0],x[:,:,:,0],x[:,:,:,0]),axis=3)#bs,224,224,3
        #print('in train step full_x shape:', full_x.shape)
        ...
        #print('in train, x shape:', full_x.shape,' ST shape:', ST.shape)    
        loss_c, loss_s,t_img_3,t_img = self.style_network(full_x,ST,alpha=1.)
    ...
    def _test_eval(self, x, lbl, styles, epoch = 0, batch = 0):
        ...
        with torch.no_grad():
            ...
            if ST.shape[0]!=full_x.shape[0]:
                ST = ST[:full_x.shape[0],:,:,:]
            #print('in eval, x shape:', full_x.shape,' ST shape:', ST.shape)
            loss_c, loss_s,t_img_3,t_img = self.style_network(full_x,ST,alpha=1.)
            #transferred_img = t_img_3.detach()
            '''
            if epoch == 0:
                for i in range(0,full_x.shape[0]):
                    tmp = np.transpose(full_x_t[i,:,:,:],(1,2,0))*255
                    tmp[:,:,0]=0
                    tmp[:,:,2]=0
                    save_filename = '0aimg_'+str(batch+i)+'.png'
                    save_filename = os.path.join(self.save_img_dir, save_filename)
                    cv2.imwrite(save_filename, tmp)
            
            for i in range(0,transferred_img.shape[0]):
                if epoch==5: continue
                if epoch%50!=0 :continue
                #if epoch%100!=0:continue
                tmp = np.transpose(transferred_img[i,:,:,:].cpu().numpy(),(1,2,0))*255
                save_filename = 'epoch'+str(epoch)+'_img'+str(batch+i)+'.png'
                save_filename = os.path.join(self.save_img_dir, save_filename)
                cv2.imwrite(save_filename, tmp)
                ...
                cv2.imwrite(save_filename, tmp)
            '''
            #X_seg = torch.zeros((sample_num,2,224,224), dtype=t_img.dtype).to(self.device)
            #X_seg[:,0:,:,:] = X_seg[:,0:,:,:]+t_img
            #y_ = self.net(X_seg)
            y_ = self.net(t_img)
            y = y_[0]
        ...
        for iepoch in range(self.n_epochs):    
            ...
            if save_path is not None:
                if iepoch==self.n_epochs-1 or iepoch%save_every==1:
                    ...
                    #this is to save to the actual path
                    '''
                    extra_savepath = '/content/drive/MyDrive/Hardvard_BC_research/cellpose-main/model_save/tmp/'
                    extra_filename = os.path.join(extra_savepath, file_name)
                    extra_filename_decoder = os.path.join(extra_savepath, file_name_decoder)
                    self.net.save_model(extra_filename)
                    print('saving net to ',extra_filename)
                    self.style_network.save_model(extra_filename_decoder)
                    print('saving adain net to ',extra_filename_decoder)
                    '''
