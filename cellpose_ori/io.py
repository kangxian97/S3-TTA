...
def load_images_labels(tdir,style_dir, mask_filter='_masks', image_filter=None, look_one_level_down=False, unet=False):
    image_names = get_image_files(tdir, mask_filter, image_filter, look_one_level_down)
    nimg = len(image_names)

    style_names = get_image_files(style_dir, mask_filter, image_filter, False)
    nstyle = len(style_names)
    # training data
    ...
    styles = []
    k = 0
    for n in range(nstyle):
        if os.path.isfile(style_names[n]):
            print(style_names[n])
            style = cv2.imread(style_names[n])
            styles.append(style)
    for n in range(nimg):
    ...
    io_logger.info(f'There are in total {nstyle} style images in style folder {style_dir} ')
    return images, labels, image_names, styles

def load_train_test_data(train_dir,style_dir, test_dir=None, image_filter=None, mask_filter='_masks',
                            unet=False, look_one_level_down=False ):
    images, labels, image_names, styles_train = load_images_labels(train_dir,style_dir, mask_filter, image_filter, look_one_level_down, unet)
                    
    # testing data
    test_images, test_labels, test_image_names = None, None, None
    if test_dir is not None:
        test_images, test_labels, test_image_names, styles_test = load_images_labels(test_dir, style_dir,mask_filter, image_filter, look_one_level_down, unet)

    return images, labels, image_names, test_images, test_labels, test_image_names, styles_train, styles_test
