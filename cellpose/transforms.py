...
def move_min_dim(img, force=False):
    ...
    img = move_axis(img, m_axis=-1, first=False)
    print(img.shape)
    return img
...
def reshape_train_test(train_data, train_labels,styles, test_data, test_labels, channels, normalize=True):
    ...
    train_data, test_data, styles,run_test = reshape_and_normalize_data(train_data,styles, test_data=test_data, 
    ...
    return train_data, train_labels, test_data, test_labels, styles, run_test

def reshape_and_normalize_data(train_data, styles, test_data=None, channels=None, normalize=True):
    ...
    run_test = False
    for test, data in enumerate([train_data, test_data,styles]):
        if data is None:
        ...
        for i in range(nimg):
            ...
            if normalize:
                data[i] = data[i]/255 #normalize_img(data[i], axis=0)
            #for style images only
            
        nchan = [data[i].shape[0] for i in range(nimg)]
    run_test = True
    return train_data, test_data, styles, run_test
...
def random_rotate_and_resize(X,styles, Y=None, scale_range=1., xy = (224,224),style_xy = (224,224), 
    ...
    """ augmentation by random rotation and resizing
        ...
        styles: List the style anchor, preprocessed
    ...
    #populate the style ndarray
    stylei = np.zeros((nstyle, 3, style_xy[0], style_xy[1]), np.float32)
    
    for ind in range(nstyle):
        style_green = styles[ind][1,:,:]
        style_green = cv2.resize(style_green, dsize=(style_xy[0], style_xy[1]), interpolation=cv2.INTER_NEAREST)
        style_img = np.stack((style_green,style_green,style_green))
        stylei[ind,:,:,:] = style_img
